...
def totalSpendPerCustomer(csv_file='../transactions.csv'): # Stores csv outside of the current directory
    ...
    # Calculates the total spend per customer in the csv file and sorts the values in descending order
    calculateTotalSpendPerCustomer = df.groupby('customer_id')['amount'].sum().sort_values(ascending=False).reset_index() # Using reset_index() so customer_id is a column, not an index
